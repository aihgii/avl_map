...
namespace not_std
{
    ...
    public:
        ...
        map(std::initializer_list<value_type> __l, const allocator_type& __a)
                : _M_t(_Compare(), _Pair_alloc_type(__a))
        { _M_t._M_insert_unique(__l.begin(), __l.end()); }
        ...
        void
        swap(map& __x)
        _GLIBCXX_NOEXCEPT_IF(std::__is_nothrow_swappable<_Compare>::value)
        { _M_t.swap(__x._M_t); }
        ...
        -> decltype(std::pair<iterator, iterator>(_M_t._M_equal_range_tr(__x)))
        { return std::pair<iterator, iterator>(_M_t._M_equal_range_tr(__x)); }
        ...
        -> decltype(std::pair<const_iterator, const_iterator>(
